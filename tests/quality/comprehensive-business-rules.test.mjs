...
test('Rules: Agenda imports TIKERAMA Cote dIvoire events through admin curation without copying media', async () => {
  ...
  assert.match(cronService, /deleteEventAttendanceRecords/);
  assert.match(cronService, /deleteAgendaEventWithAttendance/);
  assert.match(cronService, /ATTENDANCE_DELETE_PAGE_LIMIT = 450/);
  assert.match(cronService, /where\('expires_at', '<=', now\)/);
  assert.match(cronService, /throw error/);
  ...
  assert.match(adminController, /deleteAgendaEventWithAttendance/);
  assert.match(adminController, /searchTikeramaAgenda/);
  assert.match(adminController, /importTikeramaAgenda/);
  assert.match(adminController, /createAdminAgendaEvent/);
  assert.match(adminController, /poster_format:\s*'WIDE_16_9'/);
  assert.match(adminController, /CREATE_ADMIN_AGENDA_EVENT/);
  ...
  assert.match(adminRoutes, /post\('\/agenda\/events', createAdminAgendaEvent\)/);
  ...
  assert.match(adminAgendaSeeder, /Publier une affiche 16:9/);
  assert.match(adminAgendaSeeder, /uploadImageVariantsWeb/);
  assert.match(adminAgendaSeeder, /getImageRatio/);
  assert.match(adminAgendaSeeder, /\/api\/admin\/agenda\/events/);
  ...
  assert.match(webAgenda, /poster_format/);
  assert.match(webAgenda, /isWidePoster/);
...
test('Rules: Admin-user support messaging is bidirectional on web', async () => {
  ...
  assert.match(supportController, /ADMIN_RECIPIENT_QUERIES/);
  assert.match(supportController, /where\(field, '==', value\)/);
  assert.match(supportController, /\['is_admin', true\]/);
  assert.match(supportController, /\['isAdmin', true\]/);
  assert.match(supportController, /\['is_admin', 'true'\]/);
...
test('Rules: Web profile Store groups subscriptions and global purchases', async () => {
  ...
  const webChat = await read('web/src/pages/ChatPage.tsx');
  const webFilters = await read('web/src/components/FilterModal.tsx');
  const webLikesInbox = await read('web/src/pages/LikesInboxPage.tsx');
  const webPaymentReturn = await read('web/src/pages/PaymentReturnPage.tsx');
  const webStories = await read('web/src/pages/StoriesPage.tsx');
  ...
  assert.match(webApp, /to="\/store" className="text-amber-500/);
  assert.match(webApp, /path="\/premium" element=\{<Navigate to="\/store" replace \/>/);
  assert.match(webApp, /path="\/boost" element=\{<Navigate to="\/store" replace \/>/);
  assert.doesNotMatch(webApp, /const PremiumPage/);
  assert.doesNotMatch(webApp, /import\('\.\/pages\/PremiumPage'\)/);
  assert.doesNotMatch(webApp, /element=\{<PremiumPage/);
  assert.doesNotMatch(webApp, /const BoostPage/);
  assert.doesNotMatch(webApp, /import\('\.\/pages\/BoostPage'\)/);
  assert.doesNotMatch(webApp, /element=\{<BoostPage/);
  assert.match(webProfile, /navigate\('\/store'\)/);
  assert.match(webProfile, /Store Galant/);
  assert.doesNotMatch(webProfile, /navigate\('\/premium'\)/);
  ...
  assert.match(webStore, /Monthly Premium/);
  assert.match(webStore, /Priority swipes/);
  assert.match(webStore, /Premium Mensuel/);
  assert.match(webStore, /Privilege 3 Mois/);
  assert.match(webStore, /Swipes prioritaires/);
  assert.doesNotMatch(webStore, /Unlimited priority swipes/);
  ...
  assert.match(webChat, /navigate\('\/store'\)/);
  assert.doesNotMatch(webChat, /navigate\('\/premium'\)/);
  assert.match(webFilters, /navigate\('\/store'\)/);
  assert.doesNotMatch(webFilters, /navigate\('\/premium'\)/);
  assert.match(webLikesInbox, /to="\/store"/);
  assert.doesNotMatch(webLikesInbox, /to="\/premium"/);
  assert.match(webPaymentReturn, /to="\/store"/);
  assert.doesNotMatch(webPaymentReturn, /to="\/premium"/);
  assert.match(webStories, /to="\/store"/);
  assert.doesNotMatch(webStories, /to="\/premium"/);
...
test('Rules: Apps exposes paid user partner discovery with direct Google import', async () => {
  ...
  assert.match(googleMapsService, /COUNTRY_HINTS/);
  assert.match(googleMapsService, /US:\s*\{\s*country:\s*'United States'/);
  assert.match(googleMapsService, /'etats unis'/);
  assert.match(googleMapsService, /dakar:\s*COUNTRY_HINTS\.SN/);
  assert.match(googleMapsService, /douala:\s*COUNTRY_HINTS\.CM/);
  ...
  assert.match(googleMapsService, /options\.useLocationScope === true && hasCoordinates/);
  ...
  assert.match(venueController, /hasRequestedCity/);
  assert.match(venueController, /hasRequestedCity \? undefined : req\.user\.latitude/);
  assert.match(venueController, /!hasRequestedCity && !hasRequestCoordinates \? req\.user\.country : ''/);
  assert.match(venueController, /useLocationScope:\s*hasRequestCoordinates/);
  ...
  assert.match(webPartnerDiscovery, /RATING_LEVELS/);
  assert.match(webPartnerDiscovery, /category/);
  assert.match(webPartnerDiscovery, /ratingLevel/);
  assert.match(webPartnerDiscovery, /Prestige/);
  assert.match(webPartnerDiscovery, /Tres bien notes/);
  assert.match(webPartnerDiscovery, /Corrects/);
