...
test('Payments: Wave manual orders stay pending until admin validation', async () => {
  ...
  assert.match(controller, /fetchOpenPendingManualPayments/);
  ...
  assert.match(controller, /\.where\('status',\s*'==',\s*'PENDING'\)\s*\n\s*\.orderBy\('created_at',\s*'asc'\)\s*\n\s*\.limit\(batchLimit\)/);
  assert.match(controller, /while \(collected\.length < queryLimit && scanned < maxScanned\)/);
  assert.match(controller, /query = query\.startAfter\(lastDoc\)/);
  assert.match(controller, /if \(!isManualPaymentExpired\(payment,\s*nowMs\)\)/);
  assert.match(controller, /const pendingPayments = status === 'OPEN' \? await fetchOpenPendingManualPayments\(queryLimit\) : \[\]/);
  assert.match(controller, /getOpenPaymentPriority/);
  assert.match(controller, /SUBMITTED:\s*0/);
  assert.match(controller, /PROCESSING:\s*1/);
  assert.match(controller, /PENDING:\s*2/);
  assert.match(controller, /sortOpenPaymentsForAdmin/);
  assert.match(controller, /isManualPaymentExpired/);
  assert.doesNotMatch(controller, /\.filter\(item => status !== 'OPEN' \|\| item\.status !== 'PENDING' \|\| !isManualPaymentExpired\(item\)\)/);
  assert.match(controller, /\.sort\(status === 'OPEN' \? sortOpenPaymentsForAdmin : sortOldestFirst\)/);
...
test('Payments: web Store exposes Wave payment flow without screenshots', async () => {
  ...
  assert.match(finances, /PROCESSING/);
  assert.match(finances, /getOpenPaymentPriority/);
  assert.match(finances, /sortAdminPayments/);
  assert.match(finances, /statusFilter === 'OPEN'/);
