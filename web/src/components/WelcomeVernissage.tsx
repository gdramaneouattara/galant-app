...
const STEPS = [
  ...
  },
  {
    ...
    subtitle: "DÉCOUVERTE & EFFICACITÉ",
    description: "Parcourez les profils avec fluidité et comparez les meilleures offres du marché ivoirien pour vos cadeaux de prestige.",
    icon: LayoutGrid,
    color: "text-primary",
    bg: "bg-primary/10"
  },
...
const WelcomeVernissage: React.FC<WelcomeVernissageProps> = ({ onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-slate-950/95 backdrop-blur-2xl flex items-center justify-center p-6 overflow-hidden">
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="relative mx-auto w-32 h-32 flex items-center justify-center">
            ...
            <div className="absolute -inset-4 bg-white/5 rounded-full blur-3xl -z-10 animate-pulse"></div>
          </div>
          ...
          >
            {currentStep === STEPS.length - 1 ? (
              <>ENTRER DANS LE CERCLE <Check size={18} /></>
            ) : (
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] h-[150%] bg-gradient-to-br from-primary/5 via-transparent to-blue-500/5 -z-20 pointer-events-none rotate-12"></div>
    </div>
  );
};
